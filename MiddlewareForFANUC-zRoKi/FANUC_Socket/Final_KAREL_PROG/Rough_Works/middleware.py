...
#             r= requests.get(f'{ORCHESTRATOR_URL}200')
#             print(f'[X-M] Status Code: {r.status_code}, CMD: {CMD.get(cmd)}')
#             print(f'[X-M] {r.text}')
...
#             r= requests.get(f'{ORCHESTRATOR_URL}199')
#             print(f'[X-M] Status Code: {r.status_code}, CMD: {CMD.get(cmd)}')
#             print(f'[X-M] {r.text}')
...
#Position Data
# @app.route('/send-pos-data', methods=["POST"])
# def send_pos_data():
#         threading.Thread(target=connect_socket,args=(mqtt,),
#                         daemon = True).start()
#         return ("XYZWPR-CONFIG")

#FTP client
# @app.route('/send-pic', methods=["POST"])
# def send_pic():
#         threading.Thread(target=download_and_publish_pic,args=(mqtt,),
#                         daemon = True).start()
#         return ("work area")
...
#POS from zRoki
# (@app.route('/zRoki-POS', methods=["POST"])
# def zRoki_POS():
#         threading.Thread(target=download_and_publish_pic,args=(mqtt,),daemon = True).start()
#         return "work area")
