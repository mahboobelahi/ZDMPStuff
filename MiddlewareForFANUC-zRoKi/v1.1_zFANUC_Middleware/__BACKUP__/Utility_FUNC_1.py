...
#process commands
def send_(JSON_STR,client):
    threading.Thread(download_and_publish_pic,
                    args=(JSON_STR,client),).start()
    
    
# def process_CMD(cmd,client):
#     if cmd in CMD:
...
#             soc = threading.Thread(target=connect_socket, daemon=True)
#             soc.start()
...
#         elif CMD.get(cmd) == 200:
#             # run Socket Messing Server 
#             req= requests.get(f'{ORCHESTRATOR_URL}',params=payload)
            
#             print(f'[X-UF] Status Code: {req.status_code}, CMD: {CMD.get(cmd)}')
#             print(f'[X-UF] {req.text}')
#             threading.Thread(target=connect_socket,args=(client,),
#                          daemon = True).start()
#             return
